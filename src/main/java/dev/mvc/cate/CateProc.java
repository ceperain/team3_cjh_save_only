...
public class CateProc implements CateProcInter {
    ...
    public List<CateVO> list_by_cateno_search_paging(HashMap<String, Object> map) {
      ...
      */
      ...
      List<CateVO> list = this.cateDAO.list_by_cateno_search_paging(map);
      
      for (CateVO cateVO : list) { // 내용이 160자 이상이면 160자만 선택
        String contents = cateVO.getContents();
        if (contents.length() > 150) {
          contents = contents.substring(0, 150) + "...";
          cateVO.setContents(contents);
        }
        
        String name = Tool.convertChar(cateVO.getName());  // 특수 문자 변환
        cateVO.setName(name);
...
//      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    public List<StoreVO> list_search(String word) {
        List<StoreVO> list= this.cateDAO.list_search(word);
        return list;
    }
