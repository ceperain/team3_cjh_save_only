...
public class NoticeProc implements NoticeProcInter{
    ...
    public List<NoticeVO> list_search(String word) {
        List<NoticeVO> list = this.noticeDAO.list_search(word);
    ...
    public int search_count(HashMap<String, Object> hashMap) {
        int cnt = this.noticeDAO.search_count(hashMap);
        return cnt;
    }
    ...
    public List<NoticeVO> list_search_paging(HashMap<String, Object> map) {
        ...
        */
        ...
        List<NoticeVO> list = this.noticeDAO.list_search_paging(map);
...
//      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
      ...
      if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 페이지수가 11페이 이상임으로 이전 그룹으로 갈수 있는 링크 생성 
        str.append("<span class='span_box_1'><A href='"+Notice.LIST_FILE+"?&word="+word+"&now_page="+_now_page+"'>이전</A></span>"); 
      } 
      ...
      for(int i=start_page; i<=end_page; i++){ 
        ...
        if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
          ...
          str.append("<span class='span_box_1'><A href='"+Notice.LIST_FILE+"?word="+word+"&now_page="+i+"'>"+i+"</A></span>");   
        } 
      } 
      ...
      if (now_grp < total_grp){ 
        str.append("<span class='span_box_1'><A href='"+Notice.LIST_FILE+"?&word="+word+"&now_page="+_now_page+"'>다음</A></span>"); 
      } 
      str.append("</DIV>"); 
